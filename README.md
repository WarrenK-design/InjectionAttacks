...
1. NodeJS and NPM setup - Sets up the programming languages needed
2. Backend  - Goes through the installation of backend dependecies 
3. Database - Goes through the database setup. 
4. Frontend - Goes through the frontend dependencies.  
5. Running App - Goes through how to run the Frontend and Backend. 
...
A version of [NodeJS](https://nodejs.org/en/) is required to run to project, install it from there website if not already installed. 
Alternatively the [Node Version Manager](https://github.com/nvm-sh/nvm) can be used is generally recommended as different versions of NodeJS can be easily installed and seitched between. 
...
A version of npm sould be installed when NodeJS has been installed. 
To check the installation is succseful run the following commands. 
```
...
Alternatively the MariaDB server can be installed though apt  using the command line. This repository has been tested using a windows OS with MySQL server from the MySQL website and on a Raspian Linux OS using the Maria DB server. Note the SQL injection attack on the database can be performed on Windows but the command injection attack can only be performed on a Linux OS so it is reccomnded to use a Linux OS. 
...
```
Need to setup a ```root``` user, this step is recommended to secure the installation. 
...
Now you can login to the MySQL database using the MySQL cli installled with MariaDB, the command below logs in as the ```root``` user. 
```
sudo mysql -u root -p
```
...
Now need to create a user called ```SecurityAssignment``` with a password of ```Security*123```. This can really be whatever the user wants but if you change it you need to change the configuration in the ```.env``` file of the backend directory so I reccomend you dont. 
```
CREATE USER 'SecurityAssignment'@'localhost' IDENTIFIED BY 'Security*123';
```
Now need to grant all permessions for the ```SecurityAssignment``` assignment user to the ```classicmodels``` database so it can access it from the application. 
```
...
Run the command below from the ```backend``` directory to populate the database, should get the following output upon success.
```
$ npm run seedData
...
The front end is a [React](https://reactjs.org/) application and provides a way with interfacing with the data in the backend through a nice user dashboard. It was initilised using the [create react app](https://reactjs.org/docs/create-a-new-react-app.html) to get a base template. 
...
This concludes the setup of the environment, please go to the report submitted with this assignment to see how to run the attacks or details can be found on the frontend of the app. 
